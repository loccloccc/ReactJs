...
libray.display();
function searchBook(name:string, libray:Libray):void{
    let flag:number = -1;
    libray.book.forEach(e => {
            if(e.getTitle().toLowerCase().trim() === name.toLowerCase().trim()){
                console.log(`Ma : ${e.getId()} , ten sach : ${e.getTitle()} , ten Tac gia : ${e.getAuthor()}`);
                flag=0;
            }
    })
    if(flag==-1){
...
let name:string|null=prompt("nhap ten sach :") || "";

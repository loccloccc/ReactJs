...
export default function Products() {
  ...
  const handleChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    formData.append("upload_preset", import.meta.env.VITE_CLOUD_NAME);
    formData.append("cloud_name", import.meta.env.VITE_PROJECT_NAME);
    try {
      const response = await axios.post(
        `https://api.cloudinary.com/v1_1/${import.meta.env.VITE_PROJECT_NAME}/image/upload`,
        formData
      );
      console.log(response.data.url);
      setUrl(response.data.url); // giữ nguyên như bạn
    } catch (error) {
      console.log(error);
    }
  };
  ...
  function onEdit(row: ProductRow) {
    setEditing(row);
    setUrl(row.image); // hiển thị ảnh cũ khi sửa
    setOpen(true);
  }
